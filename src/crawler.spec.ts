...
const arr: EventType[] = eventsList()
  ...
  moments.forEach(async (moment) => {
    test(`crawler ${concatEventData(event, moment)}`, async () => {
      ...
      if (eventsFound == 0) {
        ...
        let initiaEventlList: Locator[] = await groupAllEvents(page.locator("ul.voohof"));
        let updatedEventList: Locator[] = await groupAllEvents(page.locator("ul.PkcPZe"));
        const mergedEventList = initiaEventlList.concat(updatedEventList);
        await logItems(mergedEventList);
      }
    });
  });
});
